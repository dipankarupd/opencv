...
def on_size_change(value):
    global cube_size, square_size
    cube_size = value
    square_size = value

# Create a black canvas with a white square (2D Image)
img_2d = np.zeros((300, 300, 3), dtype=np.uint8)

# Initial parameters for 2D Image
square_center = (150, 150)
square_size = 50
...
while True:
    # Resize 2D Image (Square)
    img_2d = np.zeros((300, 300, 3), dtype=np.uint8)
    square_top_left = (int(square_center[0] - square_size / 2), int(square_center[1] - square_size / 2))
    square_bottom_right = (int(square_center[0] + square_size / 2), int(square_center[1] + square_size / 2))
    cv2.rectangle(img_2d, square_top_left, square_bottom_right, square_color, -1)
